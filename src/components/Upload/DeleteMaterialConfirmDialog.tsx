...
function kindLabel(kind: NonNullable<Props["kind"]>): string {
  return kind === "video" ? "视频" : kind === "voice" ? "音频" : "图片";
}
    ...
    <Dialog
      ...
      footer={
        <>
          ...
          >
            <Trash2 size={14} />
            永久删除 {count} {kind === "video" ? "个视频" : kind === "voice" ? "个音频" : "张图片"}
          </Button>
        </>
      }
    >
      <div className="space-y-3 text-sm leading-6 text-text-secondary">
        <p>此操作将从微信公众号永久素材库中删除所选的 {count} {kind === "video" ? "个视频" : kind === "voice" ? "个音频" : "张图片"}，删除后无法撤销。</p>
