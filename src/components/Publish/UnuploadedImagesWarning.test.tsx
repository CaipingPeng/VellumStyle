...
test("renders associated warning text and every diagnostic category", () => {
  ...
  try {
    const warning = view.container.querySelector('[role="alert"]') as HTMLElement;
...
test("buttons invoke callbacks, expose focus ref, and continue is dangerous", () => {
  const view = render();
  try { const [back, cont] = buttons(); act(() => back.click()); act(() => cont.click());
...
test("busy disables both buttons and Escape does nothing", () => {
  ...
  try { assert.ok(buttons().every(x => x.disabled)); act(() => window.dispatchEvent(new window.KeyboardEvent("keydown", {key:"Escape"}))); assert.deepEqual(view.counts(), {backs:0, continues:0}); }
  finally { view.cleanup(); }
});
...
test("Escape backs once per event, never continues, and listener is cleaned up", () => {
  ...
  assert.deepEqual(view.counts(), {backs:1, continues:0}); view.cleanup();
  act(() => window.dispatchEvent(new window.KeyboardEvent("keydown", {key:"Escape"})));
