...
4. ⭐文章管理与云同步⭐：
   ...
   - 文章管理：在本地用户目录，针对本软件会创建一个目录用于存档我们写过的文章。如下图左图所示，文章是树形管理的，也就是说你可以像在使用windows文件管理一样在软件内`新建`、`重命名`、`删除`、`拖动文件及文件夹`，也可以通过`右键菜单在系统文件管理器中打开文件位置`。删除非空的文件夹前会提示将被递归删除的子项数量，降低误操作风险。文档侧栏宽度可以自由拖动调整；打开深层文档时，软件会自动展开其所在目录。
