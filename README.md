...
- ⭐文章管理与云同步⭐：
   ...
   - 重命名体验：`F2`、双击（文件与文件夹均可）、悬停铅笔、右键菜单都能进入改名；改名在**遮罩弹层**里完成——鼠标怎么动都不会丢掉已输入内容（点到弹层外只提示、不关闭，焦点始终留在输入框），只有 `Esc`、取消按钮或明确点「放弃修改」才退出；进入时只选中主文件名（不含 `.md`），避免误改扩展名；非法字符、系统保留名和同级重名在输入过程中即以行内提示拦下，失败会保留输入框供继续修改；文件夹改名后展开状态与选中项自动跟随。
...
- ⭐主题系统⭐：
   ...
   - 字号调整：①**全局字号**在主题弹窗里调，整篇文章等比缩放，主题的比例关系保持不变；②**直接点预览里的元素**（标题、段落、引用、表格、代码块、图注、脚注、目录……）即可单独调这一类元素的字号，被选中的元素会同时虚线描边，一眼看出改的是哪些地方。两者可叠加，且只影响本机显示，不改动主题文件本身。
