...
1. 点击左上角文档按钮打开文档树。
2. 新建或打开一篇 Markdown 文档。
3. 左侧编辑 Markdown，右侧实时预览公众号排版。
4. 使用顶部语法工具栏快速插入标题、加粗、斜体、链接、列表、引用、代码块、分割线等语法。
5. 状态栏会显示当前文档、行数、字数、排版主题、代码主题和保存状态。
...
预览时 mmbiz 图片会走 `wximg` 代理显示，复制到微信时会自动还原为原链接。

### 导入外部 Markdown

点击「导入」，选择 `.md` 或 `.markdown` 文件。导入流程会：

1. 读取 Markdown 文件。
2. 扫描 Markdown 图片、HTML `<img>` 和 Obsidian `![[...]]` 图片引用。
3. 尝试解析本地图片路径，必要时可指定资源根目录。
4. 上传本地或远程图片到微信素材库。
5. 替换 Markdown 中的图片链接。
6. 在当前文档树位置新建或覆盖同名文档并打开。

当前版本会识别视频引用，但不会自动上传或替换视频。
...
1. 等待 MathJax 公式排版完成。
2. 读取预览 DOM。
3. 给顶层元素写入 `data-tool="vellumstyle"`。
4. 还原 mmbiz 代理链接。
5. 删除同步滚动用的 `data-line` 和样式编辑用的临时 class。
6. 将 MathJax 节点转换为更适合微信编辑器的结构。
7. 使用 `juice` 将当前排版主题和代码主题 CSS 内联。
8. 以 `text/html` 写入剪贴板。
...
应用会兼容旧选择器 `#nice`、`#wechat-article`，导入或扫描时统一转换为当前文章根选择器 `#article`。
...
```bash
python scripts/crawl_mdnice_themes.py
```

该脚本是维护工具，不是日常开发必需。它依赖环境变量：

| 变量 | 说明 |
| --- | --- |
| `MDNICE_TOKEN` | mdnice 接口 Bearer token |
| `MDNICE_OUT_ID` | mdnice 文章 outId |

token 会过期，不要写入源码或 README 示例中。
...
这是正常现象。Web 模式只适合调试前端。以下能力必须使用 `npm run tauri dev`：
...
这是为了避免 WebView2 把 HTML5 拖拽事件吞掉。不要在不了解影响的情况下改回 `true`。
