...
- **Markdown 文章导入**：可一次选择一个或多个 `.md` / `.markdown` 文件导入；导入时会扫描远程图片链接，下载并上传到微信素材库后替换链接。对于本地图片链接，默认会从 Markdown 所在目录及常见附件目录自动寻址，需要时也可以在导入对话框中展开并手动指定资源根目录。
...
- **多文档管理**：应用数据目录下维护真实 `documents/` 文件树，支持新建、重命名、删除、移动和自动保存；长文件名可悬停查看全称，文件树侧栏可拖拽调宽。
...
上传图片或发布草稿前，需要在公众号后台把当前机器的出口 IP 加入 API IP 白名单。打开应用右上角「设置」，切到「网络辅助」页面后，可以点击「获取出口 IP」读取当前出口 IPv4，再点击复制按钮粘贴到公众号后台。

如果不熟悉微信后台的入口，网络辅助页面的提示文案中提供了「VellumStyle-文澜排版帮助文档」链接。桌面应用会通过系统默认浏览器打开该飞书文档，里面包含更详细的操作步骤。
...
- 横屏图片滑动容器。
- Obsidian 图片 embed 在导入流程中转换，并参与本地资源自动寻址。
...
### 导入 Markdown 后本地图片未替换

导入对话框默认会从 Markdown 文件所在目录，以及 `assets`、`images`、`attachments`、`附件` 等常见附件目录中查找本地图片。

- 如果图片统一放在其他目录，导入时勾选资源根目录选项，再选择对应目录。
- 如果多个候选目录中存在同名图片，工具会保留原引用以避免误替换；整理目录或手动指定资源根目录后再导入即可。
- 批量导入时，每个 Markdown 文件都会按自己的所在目录独立查找附件。
